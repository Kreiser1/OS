...
#define FS_TABLE_SIGNATURE (uint64_t)0x454C424154205346
#define FS_RECORD_SIGNATURE (uint64_t)0x44524F4345525346
...
{
    return ataRead(disk, FS_TABLE_OFFSET, FS_BLOCK_LENGTH, (uintptr_t)fsTable) && fsTable->Signature == FS_TABLE_SIGNATURE;
}

bool fsWriteTable(uint8_t disk, PFS_TABLE fsTable)
{
    return fsTable->Signature == FS_TABLE_SIGNATURE && ataWrite(disk, FS_TABLE_OFFSET, FS_BLOCK_LENGTH, (uintptr_t)fsTable);
}
...
{
    return ataRead(disk, recordOffset, sizeof(FS_RECORD), (uintptr_t)record) && record->Signature == FS_RECORD_SIGNATURE;
}

bool fsWriteRecord(uint8_t disk, uint64_t recordOffset, PFS_RECORD record)
{
    return record->Signature == FS_RECORD_SIGNATURE && ataWrite(disk, recordOffset, sizeof(FS_RECORD), (uintptr_t)record);
}
...
bool fsInstall(uint8_t disk, uint64_t length)
{
    ...
    fsTable.Signature = FS_TABLE_SIGNATURE;
    ...
    rootRecord.Signature = FS_RECORD_SIGNATURE;
...
{
    if (!strLength(name) || strLength(name) >= FS_NAME_LENGTH)
        return false;
    ...
    if (folder->SubRecordOffset)
    {
        ...
        fsNewRecord.Signature = FS_RECORD_SIGNATURE;
        fsNewRecord.Flags = flags;
        fsNewRecord.Length = !(fsNewRecord.Flags & FS_RECORD_FOLDER) ? length : 0;
...
{
    if (folder.Signature != FS_RECORD_SIGNATURE || !(folder.Flags & FS_RECORD_FOLDER) || !folder.SubRecordOffset)
        return false;

    if (record->Signature != FS_RECORD_SIGNATURE)
